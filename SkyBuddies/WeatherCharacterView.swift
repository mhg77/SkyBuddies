...
struct CloudCharacter: View {
    ...
    var body: some View {
        ZStack {
            ...
            // Neutral-ish smile (almost flat but slightly up)
            SmileShape(size: size * 0.2)
                .offset(y: size * 0.1)

            // Raised eyebrow left (bored)
...
struct HeavyRainCharacter: View {
    ...
    var body: some View {
        ZStack {
            ...
            // Heavy drops (tilted for wind effect)
...
struct SnowyCharacter: View {
    ...
    var body: some View {
        ZStack {
            ...
            // Chattering mouth (zig-zag)
            ChatterMouthShape(size: size * 0.2)
                .offset(y: -size * 0.02)

            // Blue-ish cold cheeks
            HStack(spacing: size * 0.3) {
...
struct FoggyCharacter: View {
    ...
    var body: some View {
        ZStack {
            ...
            // Confused face (one brow up, one down)
            HStack(spacing: size * 0.16) {
                ZStack {
                    EyeShape(size: size * 0.12)
                    // Left brow raised
                ...
                ZStack {
                    EyeShape(size: size * 0.12)
                    // Right brow lowered (normal brow)
...
/// Utility: creates a single sun ray rotated to position i out of count
@ViewBuilder
...
struct ChatterMouthShape: View {
    ...
    var body: some View {
        ZStack {
            // Zig-zag mouth (chattering teeth)
            Path { path in
                let steps = 5
